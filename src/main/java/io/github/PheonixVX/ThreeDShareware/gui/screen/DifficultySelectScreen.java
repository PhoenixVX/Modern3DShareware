...
public class DifficultySelectScreen extends Screen {
	...
	protected void init () {
		super.init();
		int w = height / 4;
		addButton(new CustomButton(width / 2 - 100, w, "Hello, NoobVille", Difficulty.PEACEFUL, true));
		addButton(new CustomButton(width / 2 - 100, w + 24, "Filthy casual!", Difficulty.EASY, true));
		addButton(new CustomButton(width / 2 - 100, w + 48, "Lemon curry?", Difficulty.NORMAL, true));
		addButton(new CustomButton(width / 2 - 100, w + 72, "eXtreme to the MaXxXxX!", Difficulty.HARD, true));
		field_19197 = addButton(new CustomButton(width / 2 - 100, w + 96, "Obligatory nightmare mode", Difficulty.HARD, true));
	}

	@Override
	public void render (MatrixStack matrices, int mouseX, int mouseY, float delta) {
		renderBackground(matrices);
		drawCenteredString(matrices, textRenderer, getTitle().asString(), width / 2, height / 4 - 60 + 20, 0xFFFFFF);
	...
	@Override
	public void onClose () {
		client.openScreen(new EpisodeSelectScreen(Text.of("Select Episode")));
	}
	...
	class CustomButton extends ButtonWidget {
		...
		public CustomButton (int n, int n2, String string, Difficulty theDifficulty, boolean bl) {
			super(n, n2, 200, 20, Text.of(string), onPress -> {
				...
				GeneratorOptions generatorOptions = new GeneratorOptions("North Carolina".hashCode(), true, true, null);
				...
				client.method_29607("3D Shareware", levelInfo2, null, generatorOptions);
			});
		}
	}
}